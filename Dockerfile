...
RUN script_path="/cron_cpu.sh" && \
    ...
    echo '  echo "$(date): CPU usage exceeded threshold ($CPU_USAGE%). Restarting application." >> /var/log/cpu_monitor.log' >> $script_path && \
    echo '  pkill -f "deno task production"' >> $script_path && \
    echo 'fi' >> $script_path && \
    chmod +x $script_path
